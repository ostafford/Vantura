...
export function showNotification(
  ...
  const container = document.querySelector<HTMLElement>('[data-notification-target="container"]')

  if (!container) {
    console.error(
      'Notification container not found. Make sure the notifications partial is rendered.'
    )
    return
  }
...
function createNotificationElement(
  ...
  wrapper.dataset.notificationAutoDismissValue = String(autoDismiss)
  wrapper.dataset.notificationDismissAfterValue = String(dismissAfter)

  const colors: Record<NotificationType, NotificationColors> = {
    success: {
      ...
      button:
        'text-success-500 dark:text-success-300 hover:text-success-700 dark:hover:text-success-500',
    },
    error: {
      bg: 'bg-white/95 dark:bg-primary-700/95 backdrop-blur-sm',
      ...
      button:
        'text-expense-500 dark:text-expense-400 hover:text-expense-700 dark:hover:text-expense-500',
    },
    warning: {
      bg: 'bg-white/95 dark:bg-primary-700/95 backdrop-blur-sm',
      ...
      button:
        'text-warning-500 dark:text-warning-400 hover:text-warning-700 dark:hover:text-warning-500',
    },
    info: {
      bg: 'bg-white/95 dark:bg-primary-700/95 backdrop-blur-sm',
