...
export default class extends Controller {
  ...
  connect() {
    // If this is a notification element (not the container), initialize it
    if (this.element.dataset.notificationType) {
  ...
  // Method to show notification from JavaScript
  // Can be called via: this.application.getControllerForElementAndIdentifier(containerElement, "notification").show(type, message)
  show(type, message) {
  ...
  createNotification(type, message) {
    ...
    wrapper.dataset.notificationAutoDismissValue = this.autoDismissValue
    wrapper.dataset.notificationDismissAfterValue = this.dismissAfterValue
        ...
        <div class="p-4">
          <div class="flex items-start">
            ...
            <div class="ml-3 w-0 flex-1 pt-0.5">
              <p class="${typeColors.text} text-sm font-medium">${message}</p>
            </div>
